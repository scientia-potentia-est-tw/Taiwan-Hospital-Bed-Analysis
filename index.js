...
function readFile() {
	fetch('https://secure-oasis-97223.herokuapp.com/bedinfo.json')
		.then(response => {
			...
			console.log(data);
			// var HospitalBedData = JSON.parse(data);
			...
			for (let amount in data) {
				...
				if (isEmpty(data[amount].info)) {
					console.log("The " + data[amount].hosp + " hospital info is empty");
					continue;
				} else {
					switch (data[amount].city) {
						case "連江縣":
							countyHospitalBed.連江縣 += data[amount].info["總數"];
							countyHospitalBedUsage.連江縣 += data[amount].info["占床數"];
							break;
						case "金門縣":
							countyHospitalBed.金門縣 += data[amount].info["總數"];
							countyHospitalBedUsage.金門縣 += data[amount].info["占床數"];
							break;
						case "宜蘭縣":
							countyHospitalBed.宜蘭縣 += data[amount].info["總數"];
							countyHospitalBedUsage.宜蘭縣 += data[amount].info["占床數"];
							break;
						case "彰化縣":
							countyHospitalBed.彰化縣 += data[amount].info["總數"];
							countyHospitalBedUsage.彰化縣 += data[amount].info["占床數"];
							break;
						case "南投縣":
							countyHospitalBed.南投縣 += data[amount].info["總數"];
							countyHospitalBedUsage.南投縣 += data[amount].info["占床數"];
							break;
						case "雲林縣":
							countyHospitalBed.雲林縣 += data[amount].info["總數"];
							countyHospitalBedUsage.雲林縣 += data[amount].info["占床數"];
							break;
						case "屏東縣":
							countyHospitalBed.屏東縣 += data[amount].info["總數"];
							countyHospitalBedUsage.屏東縣 += data[amount].info["占床數"];
							break;
						case "台東縣":
							countyHospitalBed.臺東縣 += data[amount].info["總數"];
							countyHospitalBedUsage.臺東縣 += data[amount].info["占床數"];
							break;
						case "花蓮縣":
							countyHospitalBed.花蓮縣 += data[amount].info["總數"];
							countyHospitalBedUsage.花蓮縣 += data[amount].info["占床數"];
							break;
						case "澎湖縣":
							countyHospitalBed.澎湖縣 += data[amount].info["總數"];
							countyHospitalBedUsage.澎湖縣 += data[amount].info["占床數"];
							break;
						case "基隆市":
							countyHospitalBed.基隆市 += data[amount].info["總數"];
							countyHospitalBedUsage.基隆市 += data[amount].info["占床數"];
							break;
						case "新竹市":
							countyHospitalBed.新竹市 += data[amount].info["總數"];
							countyHospitalBedUsage.新竹市 += data[amount].info["占床數"];
							break;
						case "台北市":
							countyHospitalBed.臺北市 += data[amount].info["總數"];
							countyHospitalBedUsage.臺北市 += data[amount].info["占床數"];
							// console.log(data[amount]);
							break;
						case "新北市":
							countyHospitalBed.新北市 += data[amount].info["總數"];
							countyHospitalBedUsage.新北市 += data[amount].info["占床數"];
							break;
						case "台中市":
							countyHospitalBed.臺中市 += data[amount].info["總數"];
							countyHospitalBedUsage.臺中市 += data[amount].info["占床數"];
							break;
						case "台南市":
							countyHospitalBed.臺南市 += data[amount].info["總數"];
							countyHospitalBedUsage.臺南市 += data[amount].info["占床數"];
							break;
						case "桃園市":
							countyHospitalBed.桃園市 += data[amount].info["總數"];
							countyHospitalBedUsage.桃園市 += data[amount].info["占床數"];
							break;
						case "苗栗縣":
							countyHospitalBed.苗栗縣 += data[amount].info["總數"];
							countyHospitalBedUsage.苗栗縣 += data[amount].info["占床數"];
							break;
						case "新竹縣":
							countyHospitalBed.新竹縣 += data[amount].info["總數"];
							countyHospitalBedUsage.新竹縣 += data[amount].info["占床數"];
							break;
						case "嘉義市":
							countyHospitalBed.嘉義市 += data[amount].info["總數"];
							countyHospitalBedUsage.嘉義市 += data[amount].info["占床數"];
							break;
						case "嘉義縣":
							countyHospitalBed.嘉義縣 += data[amount].info["總數"];
							countyHospitalBedUsage.嘉義縣 += data[amount].info["占床數"];
							break;
						case "高雄市":
							countyHospitalBed.高雄市 += data[amount].info["總數"];
							countyHospitalBedUsage.高雄市 += data[amount].info["占床數"];
							break;
						default:
							break;
					}
			...
			if (countyHospitalBed.高雄市 == 0) {
				countyHospitalBedLoadRate.高雄市 = 0;
			} else {
	...
	d3.json("https://raw.githubusercontent.com/scientia-potentia-est-tw/Taiwan-Hospital-Bed-Analysis/gh-pages/taiwan.json?token=260db515f58344832d1331a19e4b01b82ec50cdc", function (error, map) {
		...
		*/
		...
		d3.select("svg").selectAll("path").data(features).enter().append("path").attr({
			...
			.call(zoom);
		console.log("臺北市病床:" + countyHospitalBedLoadRate.臺北市);
		/* 上色 */
		/* 設定各縣市的數值 */
		var dataObject = {
			'連江縣': countyHospitalBedLoadRate.連江縣,
			'金門縣': countyHospitalBedLoadRate.金門縣,
			'宜蘭縣': countyHospitalBedLoadRate.宜蘭縣,
			'彰化縣': countyHospitalBedLoadRate.彰化縣,
			'南投縣': countyHospitalBedLoadRate.南投縣,
			'雲林縣': countyHospitalBedLoadRate.雲林縣,
			'屏東縣': countyHospitalBedLoadRate.屏東縣,
			'臺東縣': countyHospitalBedLoadRate.臺東縣,
			'花蓮縣': countyHospitalBedLoadRate.花蓮縣,
			'澎湖縣': countyHospitalBedLoadRate.澎湖縣,
			'基隆市': countyHospitalBedLoadRate.基隆市,
			'新竹市': countyHospitalBedLoadRate.新竹市,
			'桃園市': countyHospitalBedLoadRate.桃園市,
			'苗栗縣': countyHospitalBedLoadRate.苗栗縣,
			'新竹縣': countyHospitalBedLoadRate.新竹縣,
			'嘉義市': countyHospitalBedLoadRate.嘉義市,
			'嘉義縣': countyHospitalBedLoadRate.嘉義縣,
			'臺北市': countyHospitalBedLoadRate.臺北市,
			'新北市': countyHospitalBedLoadRate.新北市,
			'臺南市': countyHospitalBedLoadRate.臺南市,
			'高雄市': countyHospitalBedLoadRate.高雄市,
			'臺中市': countyHospitalBedLoadRate.臺中市
		};
		...
		*/
		...
		/* 利用D3.js畫出svg，顏色依照features裡的parameter參數 */
